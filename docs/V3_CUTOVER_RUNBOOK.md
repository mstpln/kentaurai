...
2. `wrangler.jsonc` points to `src/worker-v078.js` (upcoming-games app wrapper delegating to the `worker-v077.js` performance/F4 layers) and `ANALYSIS_WORKFLOW_MODE` is `v3`.
...
5. Deploy Worker v078; it must preserve the delegated worker-v077/worker-v076 v3/rollback semantics.
