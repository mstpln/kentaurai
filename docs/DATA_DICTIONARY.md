...
- C4 named trip labels are available as conservative calculated X-Labs evidence around 500 m remaining; ambiguous cases remain null and production backfill is explicit;
...
| normalized_d1 | race_positions | named race-position / running-scenario observations | rows | yes | race_positions | used | history UI / horse scenario stats / driver position rankings | source record + evidence_type + confidence + classification_version | Direct verified rows and deterministic `calculated_xlabs` rows remain distinguishable; ambiguous calculated labels are not written. |
