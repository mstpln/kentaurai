...
| Source family | Generic raw path / field | Semantic name | Data type | Nullable | Normalized target | Status | Consumer | Provenance rule | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| manual_track_profile | facts[].type + facts[].value | detailed physical track measurement | number | yes | track_profile_fact_observations | used | Bana profile / future analysis | exact track ID + source URL/type + verified_at + layout_effective_from | Supported facts are width at 1640/2140, large/first/second curve radius and first/second curve banking. Real production values stay private. |
