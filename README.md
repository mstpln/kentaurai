...
## Current build
Version 0.6.1 contains the verified official/X-Labs data foundation, resumable history pipelines, the staged external V85/V86 analysis workflow, the historical sealed-v3 stack, replay/calibration, post-race learning diagnostics, private workflow/coverage observability and the read-only PWA. Real provider payloads and private reference/editorial/contact data remain outside the public repository; GitHub contains code, migrations, tests, documentation and synthetic fixtures only.
...
2. Step 1 exports a deterministic market-blind analysis pack. The user uploads it to ChatGPT or Claude and performs the blind sports analysis without market, interviews or external editorial material.
3. Step 2 exports verified current market data for the same round. The same AI conversation compares the frozen Step 1 baseline with market ownership/odds; it does not build the system yet and does not rewrite the blind baseline.
4. Step 3 exports previously stored private external context for the round's horses/trainers. The user adds current PDFs/screenshots manually in the same AI conversation. External horse statistics, interviews and editorial opinions remain separated; driver data is not stored by this workflow.
5. The user and AI then build the final system through dialogue. The system still requires exactly three singleton spike legs.
6. External horse-statistics/interview registration is a separate optional action and can happen later. KentaurAI exports a canonical import context, the AI produces structured JSON from material already present in the conversation, and the private app validates/imports that JSON append-only. PDFs are never imported directly.
7. System registration remains a separate final action. KentaurAI validates canonical identities and exactly three singleton spike legs, then deterministically calculates row count and cost from selections and the configured line price before persistence.
8. Registered analysis/system data feeds Spel plus external-aware F1 replay and F2 post-race diagnostics. Historical sealed-v3 artifacts remain readable while their mutation paths stay disabled in the default mode.

External evidence is private. Horse context statistics are timestamped snapshots rather than mutable profile fields; current-track/balance/wagon rows are bound to KentaurAI's verified round context. Interviews are linked once to both horse and trainer/stable with the actual speaker retained. Full paid articles and provider identity never belong in the public repository.
