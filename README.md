...
Entity detail Statistik for Tränare / Hästar / Kuskar uses the same sliders-filter interaction as Trend: 1 year and Högre prissumma are the defaults, Loppnivå is a dropdown, and secondary filters stay collapsed until requested. Core statistics paint first while heavier Form/specialty reads hydrate separately.

Trend is the first primary workspace. It switches between Tränare / Hästar / Kuskar and rolling periods. Tränare and Kuskar keep the deterministic win-rate ranking with a 2-week default. Hästar ranks by the existing deterministic Form (1–100), uses 3 months and minimum 3 starts by default, and shows recent results plus factual supporting metrics. Loppnivå is a dropdown inside the shared sliders-filter panel. Trend output remains unavailable when verified inputs are insufficient; the interface never fabricates rankings.
...
Opening a round shows its eight stored legs. Horse rows expose only deterministic facts/features needed for inspection before analysis: existing horse Form, fastest plausible verified first-200 pace from the same start method, fastest plausible verified last-400 pace, market share, and expandable historical context for driver/trainer Form, rolling wins, gallop, current driver/track/start method/distance/lane category and canonical higher-prize/weekday scopes. Implausible pace observations are treated as unavailable rather than displayed as facts. No AI ranking, probability, scenario, value, spike recommendation, tips or interview/editorial evidence is introduced by this view.
