...
test('driver statistics routes are private through actual Worker', async () => {
  ...
  const url='https://example.test/app/api/drivers/statistics?period=1y&race_scope=all&race_type=all&breed_type=all&start_method=all&distance_group=all&volt_lane=all&handicap_m=all&min_starts=all';
  ...
  const data=await response.json();assert.equal(data.rankings.highestWinRate[0].id,'driver-d');assert.equal(data.definitions.longshotPercentMax,5);
  response=await worker.fetch(new Request('https://example.test/app/api/drivers/driver-d/statistics?period=1y',{headers:{cookie}}),env);assert.equal(response.status,200);
  const detail=await response.json();assert.equal(detail.summary.winRate,1);assert.equal(detail.driver.id,'driver-d');
});
...
test('actual Worker HTML composes driver Build C after Trend and horse statistics', async () => {
  ...
  for(const text of ['Flest segrar','Bäst form – senaste 30','Mest inkört i år','Bäst från spets','Bäst från dödens','Bäst med bakspår','Resultat som favorit','Resultat som skräll','Bra spår (1/6/7)']) assert.match(html,new RegExp(text.replace(/[()]/g,'\\$&')));
