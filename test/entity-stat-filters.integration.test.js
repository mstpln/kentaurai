...
function seed(db) {
  ...
  const starts = [
    { id: 'e1', race: 'r1', date: '2026-01-10', distance: 2140, method: 'auto', track: 'track-a', placing: 1, gallop: 0, mainClass: 'Silverdivisionen', firstPrize: 135000, prize: 135000 },
    { id: 'e2', race: 'r2', date: '2026-02-10', distance: 2148, method: 'volt', track: 'track-a', placing: 2, gallop: 1, mainClass: null, firstPrize: 40000, prize: 20000 },
    { id: 'e3', race: 'r3', date: '2026-03-10', distance: 1640, method: 'volte', track: 'track-b', placing: 4, gallop: 1, mainClass: null, firstPrize: 35000, prize: 5000 },
    { id: 'e4', race: 'r4', date: '2025-05-10', distance: 2140, method: 'autostart', track: 'track-b', placing: 1, gallop: 1, mainClass: 'Bronsdivisionen', firstPrize: 125000, prize: 125000 },
    { id: 'e5', race: 'r5', date: '2025-06-10', distance: 2640, method: 'voltstart', track: 'track-a', placing: 3, gallop: 0, mainClass: null, firstPrize: 30000, prize: 10000 }
  ];
  ...
  for (const start of starts) {
    ...
    db.prepare(`INSERT INTO race_results (race_entry_id, placing, result_status, gallop, prize_sek) VALUES (?, ?, 'official', ?, ?)`)
      .run(start.id, start.placing, start.gallop, start.prize);
  }
}

test('year filter applies across summary, start method, distance and track tables', async () => {
  const { env, db } = createTestEnv();
  seed(db);
  const data = await getFilteredEntityStatBreakdowns(env, 'trainers', 'trainer-filter', { year: '2026' });

  assert.equal(data.summary.starts, 3);
  assert.equal(data.summary.resultStarts, 3);
  assert.equal(data.summary.wins, 1);
  assert.equal(data.summary.seconds, 1);
  assert.equal(data.summary.thirds, 0);
  assert.equal(data.summary.top3, 2);
  assert.equal(data.summary.prizeSek, 160000);
...
test('race level applies globally and refreshes the same filtered summary', async () => {
  const { env, db } = createTestEnv();
  seed(db);

  const higherPrize = await getFilteredEntityStatBreakdowns(env, 'trainers', 'trainer-filter', { year: '2026', raceScope: 'high_prize' });
  assert.equal(higherPrize.summary.starts, 1);
  assert.equal(higherPrize.summary.wins, 1);
  assert.equal(higherPrize.summary.prizeSek, 135000);
  ...
  assert.equal(weekday.summary.starts, 2);
  assert.equal(weekday.summary.wins, 0);
  assert.equal(weekday.summary.seconds, 1);
  assert.equal(weekday.summary.prizeSek, 25000);
...
test('filtered statistics route is private and returns requested filters and summary', async () => {
  const { env, db } = createTestEnv();
  seed(db);
  ...
  assert.equal(data.summary.starts, 2);
  assert.equal(data.summary.prizeSek, 25000);
  assert.equal(data.distances.reduce((sum, row) => sum + row.starts, 0), 2);
  assert.equal(data.tracks.reduce((sum, row) => sum + row.starts, 0), 0);
});

test('statistics UI uses period and race-level controls and refreshes the summary from filtered data', () => {
  ...
  assert.match(html, /entityStatSummary/);
  assert.match(html, /summary\.innerHTML=summaryStatsFrom\(data\.summary/);
