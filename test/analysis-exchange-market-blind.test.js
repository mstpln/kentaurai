...
function seedMinimalRound(db) {
  ...
  db.prepare(`
    ...
    VALUES ('blind_round','V85','2099-06-01','2099-06-01T14:00:00Z','2099-06-01T13:55:00Z',1000000,5000000,'upcoming')
  `).run();
  for (let leg = 1; leg <= 8; leg += 1) {
    const raceId = `blind_race_${leg}`;
    const horseId = `blind_horse_${leg}`;
    const entryId = `blind_entry_${leg}`;
    db.prepare(`INSERT INTO races (id, track_id, race_date, race_number) VALUES (?, 'blind_track', '2099-06-01', ?)`).run(raceId, leg);
    ...
    db.prepare(`
      ...
      VALUES (?, ?, 'form-v2', '2099-05-31T12:00:00Z', 'form_known', 1, 'sufficient')
    `).run(`blind_feature_known_${leg}`, entryId);
    db.prepare(`
      INSERT INTO analysis_features
        (id, race_entry_id, feature_version, as_of, feature_name, numeric_value, data_quality)
      VALUES (?, ?, 'future-market-derived-v1', '2099-05-31T12:00:00Z', 'market_leak', 99, 'sufficient')
