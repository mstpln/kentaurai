...
test('D1 Step 1 excludes all interview/editorial signals from the blind pack', async () => {
  const { env, db } = createTestEnv();
  seedAsOfRound(db);
  ...
  assert.equal(pack.manifest.source_family_coverage.editorial_signals, undefined);
});
