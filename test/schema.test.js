...
const migrations = [
  ...
  '0013_combined_analysis_systems.sql','0014_official_participant_identity.sql','0015_official_participant_snapshots.sql'
];
...
test('official participant snapshot migration keeps facts timestamped, nullable and source-backed', () => {
  const db=new DatabaseSync(':memory:');db.exec(sql);
  const horseColumns=new Map(db.prepare('PRAGMA table_info(horse_official_snapshots)').all().map(r=>[r.name,r]));
  for(const required of ['horse_id','observed_at','age_years','record_code','life_starts','life_earnings_raw','source_record_id'])assert.ok(horseColumns.has(required));
  assert.equal(horseColumns.get('horse_id').notnull,1);assert.equal(horseColumns.get('observed_at').notnull,1);assert.equal(horseColumns.get('source_record_id').notnull,1);
  assert.equal(horseColumns.get('age_years').notnull,0);assert.equal(horseColumns.get('life_starts').notnull,0);
  const indexes=new Set(db.prepare("SELECT name FROM sqlite_master WHERE type='index'").all().map(r=>r.name));
  assert.ok(indexes.has('idx_horse_official_snapshots_asof'));assert.ok(indexes.has('idx_driver_official_year_snapshots_asof'));assert.ok(indexes.has('idx_trainer_official_year_snapshots_asof'));
});
