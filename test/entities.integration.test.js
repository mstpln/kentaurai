...
test('entity summary and search expose normalized read-only data', async () => {
  const { env, db } = createTestEnv(); seed(db);
  ...
  assert.equal(matches.length, 1); assert.equal(matches[0].type, 'trainer'); assert.equal(matches[0].name, 'Ada Trainer');
});

test('entity search treats SQL wildcard and escape characters literally', async () => {
  const { env, db } = createTestEnv(); seed(db);
  db.prepare(`INSERT INTO trainers (id, canonical_name, country_code) VALUES ('trainer_2','Percent% Trainer','SE'),('trainer_3','Under_score Trainer','SE'),('trainer_4','Back\\slash Trainer','SE')`).run();
  assert.deepEqual((await searchEntities(env, '%')).map((r) => r.name), ['Percent% Trainer']);
  assert.deepEqual((await searchEntities(env, '_')).map((r) => r.name), ['Under_score Trainer']);
  assert.deepEqual((await searchEntities(env, '\\')).map((r) => r.name), ['Back\\slash Trainer']);
});

test('entity lists paginate without exposing internal source ids', async () => {
  const { env, db } = createTestEnv(); seed(db);
  db.prepare(`INSERT INTO trainers (id, canonical_name, country_code) VALUES ('trainer_2','Bea Trainer','SE'),('trainer_3','Carl Trainer','SE')`).run();
  ...
  assert.equal(first.items[0].name, 'Ada Trainer'); assert.equal(first.total, 3); assert.equal(first.hasMore, true);
  assert.equal('external_id' in first.items[0], false);
  assert.equal(second.items[0].name, 'Bea Trainer'); assert.equal(second.offset, 1); assert.equal(second.hasMore, true);
  assert.equal(last.items[0].name, 'Carl Trainer'); assert.equal(last.hasMore, false);
});

test('entity pagination uses stable id tie-breakers for duplicate names', async () => {
  const { env, db } = createTestEnv(); seed(db);
  db.prepare(`INSERT INTO trainers (id, canonical_name, country_code) VALUES ('trainer_3','Same Name','SE'),('trainer_2','Same Name','SE')`).run();
  const first = await listEntities(env, 'trainers', { q: 'Same Name', limit: 1, offset: 0 });
  const second = await listEntities(env, 'trainers', { q: 'Same Name', limit: 1, offset: 1 });
  assert.equal(first.items[0].id, 'trainer_2'); assert.equal(second.items[0].id, 'trainer_3');
...
test('entity details preserve factual nulls and database activity stats', async () => {
  ...
  assert.equal(horse.entity.name, 'Comet Horse'); assert.equal(horse.entity.trainer_name, 'Ada Trainer');
  assert.equal(horse.entity.career_earnings_sek, 123456); assert.equal(horse.starts[0].placing, null);
  const trainer = await getEntityDetail(env, 'trainers', 'trainer_1');
  assert.equal(trainer.stats.databaseStarts, 1); assert.equal(trainer.stats.linkedHorses, 1);
  assert.equal(trainer.stats.resultStarts, 0); assert.equal(trainer.stats.winRate, null);
  assert.equal(trainer.starts[0].horse_name, 'Comet Horse');
});

test('scratched declarations are excluded from starts, performance and coverage', async () => {
  const { env, db } = createTestEnv(); seed(db);
  ...
  db.prepare(`INSERT INTO game_rounds (id, game_type, round_date, status) VALUES ('round_2','V85','2099-01-03','finished')`).run();
  db.prepare(`INSERT INTO game_legs (game_round_id, leg_number, race_id) VALUES ('round_2',1,'race_2')`).run();
  db.prepare(`INSERT INTO betting_snapshots (id, game_round_id, leg_number, race_entry_id, captured_at, bet_percent, market_rank) VALUES ('bet_scratched','round_2',1,'entry_2','2099-01-03T10:00:00Z',0.2,2)`).run();
  const detail = await getEntityDetail(env, 'trainers', 'trainer_1');
  assert.equal(detail.stats.databaseStarts, 1); assert.equal(detail.stats.scratchedEntries, 1);
  assert.equal(detail.coverage.starts, 1); assert.equal(detail.coverage.startsWithMarket, 0);
...
test('entity detail exposes each existing measurement family with histories and coverage', async () => {
  const { env, db } = createTestEnv(); seed(db);
  db.prepare(`INSERT INTO source_records (id, source_type, fetched_at, quality_status) VALUES ('src_1','synthetic','2099-01-01T10:00:00Z','verified'),('src_2','synthetic','2099-01-01T11:00:00Z','verified')`).run();
  ...
  db.prepare(`INSERT INTO betting_snapshots (id, game_round_id, leg_number, race_entry_id, captured_at, bet_percent, market_rank) VALUES ('bet_1','round_1',1,'entry_1','2099-01-01T10:00:00Z',0.30,2),('bet_2','round_1',1,'entry_1','2099-01-01T11:00:00Z',0.35,1)`).run();
  db.prepare(`INSERT INTO odds_snapshots (id, race_entry_id, captured_at, market_type, odds) VALUES ('odds_1','entry_1','2099-01-01T10:00:00Z','vinnare',3.0),('odds_2','entry_1','2099-01-01T11:00:00Z','vinnare',2.5)`).run();
  db.prepare(`INSERT INTO equipment (id, race_entry_id, shoes_front, shoes_rear, barefoot_front, barefoot_rear, sulky_type, exact_sulky, change_from_previous_json, verification_status, source_record_id) VALUES ('eq_1','entry_1','shod','shod',0,0,'standard','S1','{"sulkyTypeChanged":false}','reported','src_1'),('eq_2','entry_1','barefoot','shod',1,0,'bike','B1','{"shoesFrontChanged":true,"sulkyTypeChanged":true}','reported','src_2')`).run();
  ...
  assert.equal(detail.latestObservation.fields.ageYears, 5); assert.equal(detail.stats.wins, 1); assert.equal(detail.stats.v85Starts, 1);
  assert.equal(detail.breakdowns.startMethods[0].wins, 1);
  for (const key of ['startsWithMarket','startsWithOdds','startsWithEquipment','startsWithXLabs','startsWithPositions','startsWithFeatures','startsWithAi','startsWithEditorial','startsWithConditions']) assert.equal(detail.coverage[key], 1, key);
  const start = detail.starts[0];
  assert.equal(start.race_name, 'Synthetic Cup'); assert.deepEqual(start.class_flags, { final: true });
  assert.equal(start.betting.betPercent, 0.35); assert.equal(start.bettingHistory.length, 2);
  assert.equal(start.odds[0].odds, 2.5); assert.equal(start.oddsHistory.length, 2);
  assert.equal(start.equipment.sulkyType, 'bike'); assert.equal(start.equipmentHistory.length, 2);
  assert.deepEqual(start.xlabs.segments, { segment: 'synthetic' }); assert.equal(start.positions[0].leader, true);
  assert.deepEqual(start.positions[0].event, { tempo: 'synthetic' }); assert.equal(start.features[0].name, 'synthetic_feature');
  assert.equal(start.aiAnalyses[0].abcdGroup, 'A'); assert.equal(start.editorialSignals[0].signalType, 'form');
