...
function addRace(db,id,date,{method='auto',lane=1,handicap=0,actual=2140,placing=1,monte=false}={}) {
  const raceName = monte ? 'Montélopp' : 'Rest race';
  db.prepare("INSERT INTO races (id,track_id,race_date,race_number,distance_m,start_method,first_prize_sek,race_name,source_quality) VALUES (?, 'track-r', ?, 1, 2140, ?, 50000, ?, 'verified')").run(id,date,method,raceName);
