...
test('external workflow exposes independent Step 1, Step 2 and registration contracts', async () => {
  ...
  const step1Prompt = getExternalAnalysisStep1Prompt('openai');
  assert.match(step1Prompt, /Marknadsblind sportslig analys/);
  assert.match(step1Prompt, /Bygg inget spelsystem/);
  assert.match(step1Prompt, /track_analysis/);
  assert.match(step1Prompt, /race_priors/);
  assert.match(step1Prompt, /person_context/);
  assert.match(step1Prompt, /equipment_response/);
  assert.match(step1Prompt, /Räkna inte samma information flera gånger/);
  assert.match(step1Prompt, /Baseline/);
  assert.match(step1Prompt, /Startpoäng är inte samma sak som startsnabbhet/);
  assert.match(step1Prompt, /500m_after_start och 500m_remaining/);
  assert.match(step1Prompt, /ANTI-FLATTENING-KONTROLL/);
  assert.match(step1Prompt, /35 %, 45 %, 55 % eller ännu högre/);
  assert.match(step1Prompt, /Trolig positionering när loppet satt sig/);
  assert.match(step1Prompt, /Rygg ledaren/);
  assert.match(step1Prompt, /3:e invändigt/);
  assert.match(step1Prompt, /Dödens/);
  assert.match(step1Prompt, /2:a ytter/);
  assert.match(step1Prompt, /3:e ytter/);
  assert.match(step1Prompt, /Steg 2 får inte skriva om Steg 1/);
  assert.match(step1Prompt, /Använd inte intervjuer, krönikor/);
  assert.match(step1Prompt, /Sök inte på webben/);
