...
- System spike validation is keyed by game type plus system type: V85 `main` may contain two or three spike legs; a two-spike V85 main requires a clear explanation in `notes`. All other V85/V86 systems require exactly three spike legs.
- Default main-system budget is 150-250 SEK unless a later explicit user instruction changes it. The current V85 analysis method may explicitly request a larger shared main system; validation must follow the active product method rather than infer system type from budget.
