...
- The default live workflow is external-AI-first: deterministic market-blind Step 1 -> market-only Step 2 -> interviews/external-statistics Step 3 -> user/AI system dialogue -> separate external-evidence registration when desired -> final system registration.
- Step 1 is not imported or server-sealed in the normal live workflow. Once market data is shown, its probabilities, ranking and ABCD are treated as the blind baseline and must not be silently rewritten because of the market. Interviews, columns and external statistics must not enter Step 1; they belong only in Step 3 after market analysis.
...
- Manual editorial material is qualitative context, not the quantitative anchor. External horse-statistics snapshots and interview history are private, append-only evidence; no driver data is stored by that workflow.
