...
export async function loadHorsePersonHistory(env, horseId, cutoffValue) {
  if (!horseId) return [];
  ...
  const { results } = await env.DB.prepare(`
    ...
    WHERE re.horse_id=? AND re.scratched=0 AND rr.result_status='official'
  ...
  `).bind(horseId,lower,cutoff.iso,cutoff.iso).all();
