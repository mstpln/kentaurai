...
export async function loadDriverHistory(env, driverId, cutoffValue) {
  if (!driverId) return [];
  ...
  const { results } = await env.DB.prepare(`
    ...
    WHERE re.driver_id=? AND re.scratched=0 AND rr.result_status='official'
  ...
  `).bind(driverId,lower,cutoff.iso,cutoff.iso).all();
