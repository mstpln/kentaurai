...
import { enhanceExternalEvidenceUiHtml } from './app-external-evidence-ui.js';
import {
...
async function enhancedAppResponse(request, response) {
  ...
  const body = enhanceExternalEvidenceUiHtml(removeLegacyAnalysisUiScripts(await response.text()));
