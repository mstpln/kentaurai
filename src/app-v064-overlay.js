...
function v064Client(STL_OPTIONS, RACE_TYPE_OPTIONS) {
  ...
  function filtersFor(id) {
    if (!classFilters.has(id)) classFilters.set(id, { stlClass: 'all', raceType: 'all' });
    return classFilters.get(id);
  }
  function labelFor(options, value, allLabel) {
    if (!value || value === 'all') return allLabel;
    return options.find(([key]) => key === value)?.[1] || allLabel;
  }
  ...
  window.fetch = function enhancedFetch(input, init) {
    try {
      ...
      if (match) {
        ...
        if (filters.stlClass && filters.stlClass !== 'all') url.searchParams.set('stl_class', filters.stlClass);
  ...
  function rerunTrackStats() {
    const active = document.querySelector('[data-track-distance].active') || document.querySelector('[data-track-distance]');
  ...
  function openSheet(kind, options, title, allLabel) {
    ...
    if (!id) return;
    const filters = filtersFor(id);
    const value = kind === 'stlClass' ? filters.stlClass : filters.raceType;
    closeSheet();
    const backdrop = document.createElement('div');
    backdrop.className = 'kentaur-filter-sheet-backdrop';
    const panel = document.createElement('div');
    panel.className = 'kentaur-filter-sheet';
    panel.innerHTML = '<div class="kentaur-filter-sheet-head"><span>' + title + '</span><button class="kentaur-filter-sheet-close" type="button">Stäng</button></div>';
    for (const [key, label] of [['all', allLabel], ...options]) {
      const button = document.createElement('button');
      button.type = 'button';
      button.className = 'kentaur-filter-option' + (String(value) === String(key) ? ' selected' : '');
      button.innerHTML = '<span>' + label + '</span><span>' + (String(value) === String(key) ? '✓' : '') + '</span>';
      button.onclick = () => { filters[kind] = key; closeSheet(); rerunTrackStats(); };
      panel.appendChild(button);
    }
    backdrop.appendChild(panel);
    backdrop.addEventListener('click', (event) => { if (event.target === backdrop) closeSheet(); });
    panel.querySelector('.kentaur-filter-sheet-close').onclick = closeSheet;
    document.body.appendChild(backdrop);
  }
  function selectMarkup(kind, options, allLabel) {
    const id = currentTrackId();
    const filters = filtersFor(id || 'none');
    const value = filters[kind] || 'all';
    const active = value !== 'all';
    const optionsHtml = [['all', allLabel], ...options].map(([key, label]) => '<option value="' + key + '" ' + (key === value ? 'selected' : '') + '>' + label + '</option>').join('');
    return '<div class="track-select-wrap"><select class="track-select ' + (active ? 'active' : '') + '" data-track-class-select="' + kind + '">' + optionsHtml + '</select><button type="button" class="track-select-mobile ' + (active ? 'active' : '') + '" data-track-class-mobile="' + kind + '"><span>' + labelFor(options, value, allLabel) + '</span><span>⌄</span></button></div>';
  }
  function enhanceTrackFilters() {
    const stack = document.querySelector('.track-filter-stack');
    if (!stack || stack.querySelector('.track-class-filter-line')) return;
    const line = document.createElement('div');
    line.className = 'track-filter-line track-class-filter-line';
    line.innerHTML = '<div class="filter-block"><div class="filter-label">STL-klass</div>' + selectMarkup('stlClass', STL_OPTIONS, 'Alla STL-klasser') + '</div><div class="filter-block"><div class="filter-label">Lopptyp</div>' + selectMarkup('raceType', RACE_TYPE_OPTIONS, 'Alla lopptyper') + '</div>';
    stack.appendChild(line);
    line.querySelectorAll('[data-track-class-select]').forEach((select) => {
      select.onchange = () => {
        const id = currentTrackId();
        if (!id) return;
        filtersFor(id)[select.dataset.trackClassSelect] = select.value;
        rerunTrackStats();
      };
    });
    line.querySelectorAll('[data-track-class-mobile]').forEach((button) => {
      button.onclick = () => {
        const kind = button.dataset.trackClassMobile;
        if (kind === 'stlClass') openSheet(kind, STL_OPTIONS, 'STL-klass', 'Alla STL-klasser');
        else openSheet(kind, RACE_TYPE_OPTIONS, 'Lopptyp', 'Alla lopptyper');
      };
    });
  }
  ...
  async function enhanceTrackContact() {
    const id = currentTrackId();
    if (!id || document.querySelector('.track-contact-section') || contactLoads.has(id)) return;
    const body = document.getElementById('trackTabBody');
    if (!body || !document.querySelector('[data-track-tab="overview"].active')) return;
    contactLoads.add(id);
    try {
      const response = await originalFetch('/app/api/tracks/' + encodeURIComponent(id), { headers: { accept: 'application/json' } });
      if (!response.ok) return;
      const detail = await response.json();
      if (currentTrackId() !== id || document.querySelector('.track-contact-section')) return;
      const address = [detail.address?.street, detail.address?.postalCode, detail.city].filter(Boolean).join(', ');
      if (!address && !detail.websiteUrl) return;
      const section = document.createElement('section');
      section.className = 'data-section track-contact-section';
      const addressHtml = address ? '<div class="data-item"><div class="data-label">Adress</div><div class="data-value">' + escapeHtml(address) + '</div></div>' : '';
      const websiteHtml = detail.websiteUrl ? '<div class="data-item"><div class="data-label">Hemsida</div><div class="data-value"><a class="track-contact-link" target="_blank" rel="noopener noreferrer" href="' + escapeHtml(detail.websiteUrl) + '">Öppna hemsida ↗</a></div></div>' : '';
      section.innerHTML = '<div class="data-section-head"><h2>Kontakt & plats</h2></div><div class="data-grid">' + addressHtml + websiteHtml + '</div>';
      const groups = body.querySelector('.data-groups') || body;
...
function browserScript() {
  return `<script id="kentaurai-v064-overlay-script">(${v064Client.toString()})(${JSON.stringify(STL_CLASS_OPTIONS)},${JSON.stringify(RACE_TYPE_OPTIONS)});</script>`;
}
