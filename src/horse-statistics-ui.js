...
function toolbar(includeMinimum=true){return '<div class="horse-stats-toolbar"><div class="horse-stats-filter-grid">'+field('Period','period',PERIODS)+field('Loppnivå','raceScope',SCOPES)+field('Bana','trackId',trackRows())+field('Lopptyp','raceType',TYPES)+field('Rastyp','breedType',BREEDS)+field('Startmetod','startMethod',METHODS)+field('Distans','distanceGroup',distanceRows())+field('Kön','sex',SEXES)+field('Ålder','age',ageRows())+(includeMinimum?field('Min starter','minStarts',MINIMUMS):'')+'</div></div>'}
...
function bindFilters(callback){document.querySelectorAll('[data-hs-filter]').forEach(select=>select.onchange=()=>{state.horseStatsFilters[select.dataset.hsFilter]=select.value;callback()})}
...
function rankingGrid(data){const r=data.rankings||{};return '<div class="horse-ranking-grid">'+
 ...
 card('Bäst form – senaste 10','Endast officiella placeringar',r.bestFormLast10,x=>numberValue(x.averagePlacing,2)+'<span class="horse-ranking-note">'+num(x.usedStarts)+' starter</span>')+
...
async function renderHorseRankings(){const token=++rankingToken;state.detail=null;state.page='horses';setNav('horses');app.innerHTML=heading('Hästar','Sök och utforska hästar')+tabs([['list','Lista'],['stats','Statistik']],state.tab)+'<div class="horse-ranking-empty">Läser filter…</div>';await loadOptions();if(token!==rankingToken||state.page!=='horses'||state.tab!=='stats'||state.detail)return;app.innerHTML=heading('Hästar','Sök och utforska hästar')+tabs([['list','Lista'],['stats','Statistik']],state.tab)+toolbar()+'<div class="horse-ranking-empty">Läser häststatistik…</div>';bindTabs(()=>renderEntityList('horses'));bindFilters(renderHorseRankings);try{const data=await api('/horses/statistics?'+query());if(token!==rankingToken||state.page!=='horses'||state.tab!=='stats'||state.detail)return;app.innerHTML=heading('Hästar','Sök och utforska hästar')+tabs([['list','Lista'],['stats','Statistik']],state.tab)+toolbar()+rankingGrid(data);bindTabs(()=>renderEntityList('horses'));bindFilters(renderHorseRankings);bindRows()}catch(err){if(token===rankingToken&&state.page==='horses'&&state.tab==='stats'&&!state.detail)app.innerHTML+='<div class="horse-ranking-empty">Kunde inte läsa häststatistik: '+esc(err.message)+'</div>'}}
