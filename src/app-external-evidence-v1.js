function evidenceUiClient(){
  ...
  async function renderExternalStats(id){
    ...
    const groups=new Map();rows.forEach(function(r){const key=(r.contextType||'')+'|'+(r.contextKey||'');if(!groups.has(key))groups.set(key,[]);groups.get(key).push(r)});
    let html='<div class="data-groups">';
    groups.forEach(function(items){const first=items[0];html+='<section class="data-section"><div class="data-section-head"><h2>'+e(first.contextLabel||first.contextType)+'</h2><span>'+items.length+' snapshot'+(items.length===1?'':'s')+'</span></div><div class="evidence-table-wrap"><table class="evidence-table"><thead><tr><th>Observerad</th><th>Starter</th><th>Placering</th><th>Seger %</th><th>ROI</th></tr></thead><tbody>'+items.map(function(r){return '<tr><td>'+e(dateText(r.observed_at||r.available_at))+'</td><td>'+e(r.starts)+'</td><td>'+e([r.wins,r.seconds,r.thirds].map(function(v){return v==null?'—':v}).join('-'))+'</td><td>'+e(pctText(r.win_percent))+'</td><td>'+e(pctText(r.roi_percent))+'</td></tr>'}).join('')+'</tbody></table></div></section>'});
