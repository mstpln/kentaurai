...
.trend-detail-panel{display:grid;grid-template-columns:repeat(5,minmax(0,1fr)) auto;gap:10px;align-items:end;padding:13px 0 4px;border-top:1px solid var(--line-soft)}
...
.trend-win-block{border-right:1px solid var(--line);display:flex;flex-direction:column;justify-content:center;align-items:center;min-width:0;padding:0 10px}.trend-win-value{font-size:23px;line-height:1;font-weight:700;color:var(--accent-soft);font-variant-numeric:tabular-nums;white-space:nowrap}
...
function recentPlacings(item){const rows=(item.recentPlacings||[]).filter(v=>v!=null).map(v=>String(v));return rows.length?rows.join('–'):'—'}
function trendRows(items){if(!items.length)return '<div class="trend-no-data"><strong>Ingen statistik för valt urval</strong>Rankingen fylls automatiskt när verifierade resultat finns inom filtren.</div>';const horses=state.trendCategory==='horses';return '<div class="trend-results">'+items.map(item=>'<button type="button" class="trend-result-row" data-trend-id="'+esc(item.id)+'"><span class="trend-win-block">'+(horses?'<span class="trend-panel-kicker">Form</span><span class="trend-win-value">'+esc(item.formScore==null?'—':num(item.formScore))+'</span>':'<span class="trend-win-value">'+esc(trendPct(item.winRate))+'</span>')+'</span><span class="trend-result-main"><span class="trend-result-name">'+esc(item.name)+'</span><span class="trend-result-core">'+(horses?'<span>Senaste '+esc(recentPlacings(item))+'</span><span>Starter '+num(item.starts)+'</span><span>Vinster '+num(item.wins)+'</span>':'<span>Starter '+num(item.starts)+'</span><span>Vinster '+num(item.wins)+'</span><span>Förluster '+num(item.losses)+'</span>')+'</span><span class="trend-result-pills">'+(horses?metricPill('Seger %',trendPct(item.winRate))+metricPill('Topp 3%',trendPct(item.top3Rate))+metricPill('Prispengar',trendMoney(item.prizeSek)):metricPill('Topp 3%',trendPct(item.top3Rate))+metricPill('Galopp%',trendPct(item.gallopRate))+metricPill('Prispengar',trendMoney(item.prizeSek)))+'</span></span></button>').join('')+'</div>'}
