...
.trend-period-select{appearance:none;-webkit-appearance:none;border:0;background:transparent;color:var(--accent-soft);font:inherit;font-size:13px;font-weight:650;line-height:1.2;padding:3px 18px 3px 0;cursor:pointer;outline:none;max-width:130px}
...
function trendRows(items){if(!items.length)return '<div class="trend-no-data"><strong>Ingen statistik för valt urval</strong>Rankingen fylls automatiskt när verifierade resultat finns inom filtren.</div>';return '<div class="trend-results">'+items.map(item=>'<button type="button" class="trend-result-row" data-trend-id="'+esc(item.id)+'"><span class="trend-win-block"><span class="trend-win-value">'+esc(trendPct(item.winRate))+'</span></span><span class="trend-result-main"><span class="trend-result-name">'+esc(item.name)+'</span><span class="trend-result-core"><span>Starter '+num(item.starts)+'</span><span>Vinster '+num(item.wins)+'</span><span>Förluster '+num(item.losses)+'</span></span><span class="trend-result-pills">'+metricPill('Topp 3%',trendPct(item.top3Rate))+metricPill('Galopp%',trendPct(item.gallopRate))+metricPill('Prispengar',trendMoney(item.prizeSek))+'</span></span></button>').join('')+'</div>'}
